...
class ReuseQuery  : public AbstractGraph
{

    public:
    ReuseQuery(std::string filename, bool directed = false) : AbstractGraph(directed)
    {
        GraphIO::ReadDataFile(filename, directed, vertex_count_, edge_count_, row_ptrs_, cols_);
        reorder();
        BuildRestriction();
    ...
    uintV *GetRestriction() const { return restriction_; }
    bool ExistCentral() {return exist_central_node;}
    uintV* GetReuse() const {return reuse_;}
    bool* GetIsReused() const {return is_reused_;}
...
protected:
    ...
    void BuildReuse()
    {
        reuse_ =  new uintV[vertex_count_];
        is_reused_ = new bool[vertex_count_];
        for (uintV i = 0; i < vertex_count_; i++)
        {
            int max_reuse = 1;
            uintV reuse_vertex = 0xFF;
            for (uintV j = 0; j < i; j++)
            {
                bool is_subset = true;
                int reuseable = 0;
                for (uintV k = row_ptrs_[j]; k < row_ptrs_[j + 1]; k++)
                {
                    if ((cols_[k] & 0x7f) > j) continue;
                    bool exist = false;
                    reuseable ++;
                    for (uintV r = row_ptrs_[i]; r < row_ptrs_[i + 1]; r++)
                    {
                        if ((cols_[r] & 0x7f) == (cols_[k] & 0x7f)) exist = true;
                    }
                    if (!exist) is_subset = false;
                }
                if (is_subset) {
                    if (reuseable > max_reuse) {
                        max_reuse = reuseable;
                        reuse_vertex = j;
                    }
                }
            }
            reuse_[i] = reuse_vertex;
            is_reused_[reuse_vertex] = 1;
        }
    }
};
